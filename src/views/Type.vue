<template>
  <div>
      ...
      <el-col :span="24">
        <div>
          <el-table :data="dataList" ref="multipleTable" style="font-weight: bold; font-size: 18px"
            ...
            <el-table-column property="sId" label="ID" key="sId"/>
            <el-table-column property="sName" label="尺寸名" key="sName"/>
            <el-table-column property="sCode" label="尺寸编号" key="sCode"/>
                  ...
                  >
                    <template #reference>
                      <el-button type="danger"
                                 @click="this.ensureTitle = '删除' + this.dataList[scope.$index].sName + '?' ">删除
                      </el-button>
                    </template>
                  </el-popconfirm>
                </el-col>
    ...
    <el-dialog v-model="dialogFormVisible" title="新增尺寸" :before-close="hideCommit">
    ...
    <el-dialog v-model="dialogChangeVisible" title="修改尺寸" :before-close="hideChange">
      <el-form ref="formRef" :model="changeList" label-width="120px">
        <el-form-item label="尺寸名称">
          <el-input v-model="changeList.sName"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
      <span class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    queryData() {
      // get 页面和公司名
      request.get("/sys_size/detail" + "?" + "pageNum=" + this.pageNum + "&&" + "pageSize=" + this.pageSize + "&&" + "sCompany=" + this.$store.state.currentCompany.companyCode).then(res => {
        console.log(res);
    ...
    async commitAdd() {
      ...
      if (this.form.name === "" || this.form.name === undefined) {
        this.msgAlert("失败", "请填写尺寸名称！", "error")
        return
      }

        this.loading = true
          let formData = new FormData();
          formData.set("sName", this.form.name)
          formData.set("sCompany", this.$store.state.currentCompany.companyCode)

          await request.post("/sys_size/add", formData).then(res => {
            this.loading = false
            if (res.code === 100) {
                this.loading = false
                this.form = []
    ...
    async commitChange() {
      ...
      this.loading = true
      let formData = new FormData();
      formData.set("sId", this.changeList.sId)
      formData.set("sName", this.changeList.sName)
      formData.set("sCompany", this.$store.state.currentCompany.companyCode)

      await request.post("/sys_size/change", formData).then(res => {
        this.loading = false
        if (res.code === 100) {
          this.loading = false
          this.changeList = []
    ...
    // 删除按钮
    clickDelete(item) {
      request.get("/sys_size/delete" + "?" + "sId=" + this.dataList[item].sId + "&&" + "sCompany=" + this.$store.state.currentCompany.companyCode).then(res => {
        this.queryData()
      })
    },
